...
# Configuration (anchor to repo root)
REPO_ROOT = Path(__file__).resolve().parents[2]
BASE_DIR = REPO_ROOT / "src" / "analysis" / "replication" / "output"
...
class PerfectReplicationEngine:
    ...
    def calculate_growth_rate_of_capital(self, df, K_unified):
        """Calculate gK using S&T definition In/K* when possible, with ΔK/K as diagnostic.

        K* is taken as KK for 1958–1973 and K for 1974–1989 (book's capital concept per period).
        In is net investment consistent with that K* concept. As a minimal faithful proxy using
        available authentic columns, we try:
          - If 'I' exists for the period where K* is KK/K, treat it as net investment (book column label suggests fixed investment). If both 'I' and 'I!' exist, prefer 'I'.
          - Else, fall back to ΔK* (change in K*) as net investment proxy.
        """
        print("Calculating growth rate of capital (S&T In/K* where possible)…")

        index = df.index
        gk_exact = pd.Series(index=index, dtype=float, name='gK_exact_In_over_Kstar')
        gk_delta = pd.Series(index=index, dtype=float, name='gK_deltaK_over_K')

        # Build K* from book-period columns directly (avoid any reinterpretation)
        K_star = pd.Series(index=index, dtype=float, name='K_star')
        if 'KK' in df.columns:
            K_star = K_star.fillna(df['KK'])
        if 'K' in df.columns:
            K_star = K_star.fillna(df['K'])

        # Net investment series candidate from book
        I_net = None
        if 'I' in df.columns:
            I_net = df['I']
        ...
        # Compute ΔK/K diagnostic on unified K
        for i in range(1, len(K_unified)):
            y = K_unified.index[i]
            prev = K_unified.iloc[i-1]
            cur = K_unified.iloc[i]
            if pd.notna(cur) and pd.notna(prev) and prev != 0:
                gk_delta.loc[y] = (cur - prev) / prev

        # Compute In/K* where both numerator and denominator present and denominator != 0
        if I_net is not None:
            mask = K_star.notna() & (K_star != 0) & I_net.notna()
            gk_exact.loc[mask] = I_net.loc[mask] / K_star.loc[mask]
            self.methodology_notes.append(
                f"gK (exact) calculated as In/K* using book I over K* for {int(mask.sum())} years"
            )
        else:
            self.methodology_notes.append("Book investment column ('I' or 'I!') not found; exact In/K* unavailable.")

        # Prefer exact where available; otherwise fall back to ΔK/K diagnostic
        gK_preferred = gk_exact.copy()
        fill_mask = gK_preferred.isna() & gk_delta.notna()
        gK_preferred.loc[fill_mask] = gk_delta.loc[fill_mask]

        # Track counts
        self.methodology_notes.append(
            f"gK diagnostic (ΔK/K) available for {int(gk_delta.notna().sum())} years; preferred gK coverage: {int(gK_preferred.notna().sum())} years"
        )

        return gK_preferred, gk_exact, gk_delta, K_star
    ...
    def create_perfect_table(self, df_original, K_unified, u_corrected, r_perfect, gK_preferred, derived):
        ...
        df_perfect['gK_perfect'] = gK_preferred
...
"""
    ...
    def run_perfect_replication(self):
        ...
        gK_preferred, gk_exact, gk_delta, K_star = self.calculate_growth_rate_of_capital(df_original, K_unified)
        ...
        df_perfect = self.create_perfect_table(df_original, K_unified, u_corrected,
                                               r_perfect, gK_preferred, derived)
        ...
        for year in df_perfect.index:
            ...
            # Add auxiliary gK components for transparency
            row['K_star'] = K_star.loc[year] if year in K_star.index else np.nan
            row['gK_exact_In_over_Kstar'] = gk_exact.loc[year] if year in gk_exact.index else np.nan
            row['gK_deltaK_over_K'] = gk_delta.loc[year] if year in gk_delta.index else np.nan
